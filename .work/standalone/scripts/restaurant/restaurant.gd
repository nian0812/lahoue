...
func _get_upgrade_table_position(table_number: int) -> Vector2:
	...
	var result: Vector2
	if roof_index >= 0:
		result = Vector2(-104, -165) + Vector2(48, -18) * (roof_index % 3) + Vector2(43, 21) * (roof_index / 3)
	else:
		var ground_index: int = [1, 2, 3, 4, 8, 12].find(table_number)
		result = Vector2(-100 + (ground_index % 3) * 78, 35 + (ground_index / 3) * 60)
...
func get_table_approach(table_id: String) -> Array[Vector2]:
	if rooftop_tables.has(table_id.trim_prefix("table_").to_int()):
		return [Vector2(205, 80), Vector2(210, 38), Vector2(155, -127), Vector2(128, -137)]
	return [Vector2(-175, 100)]
