...
func _refresh_visual() -> void:
	...
	if visual_root != null and visual_root.has_method("set_artwork_modulate"):
		...
		# Dining modules in the supplied atlas already contain finished meals.
		# Keep the empty geometric table fallback until clean furniture is supplied.
		var approved: bool = visual_root.use_approved("restaurant/table_empty")
		visual_root.call("set_artwork_visible", approved)
		table_visual.visible = not approved
		table_visual.polygon = PackedVector2Array([Vector2(-23,0),Vector2(0,-12),Vector2(23,0),Vector2(0,12)])
	queue_redraw()


func _draw() -> void:
	if get_node("VisualRoot").approved_id == "restaurant/table_empty": return
	for at: Vector2 in [Vector2(-15,-8),Vector2(14,13)]:
		draw_colored_polygon(PackedVector2Array([at+Vector2(-6,-3),at+Vector2(0,-6),at+Vector2(6,-3),at+Vector2(0,1)]),Color("#715035"))
		draw_line(at+Vector2(0,1),at+Vector2(0,9),Color("#4e3929"),3.0,true)
	for at: Vector2 in [Vector2(-17,2),Vector2(17,2),Vector2(0,9)]:
		draw_line(at,at+Vector2(0,13),Color("#4e3929"),3.0,true)
