...
func refresh() -> void:
	_refresh_list()
	if migration_button != null:
		migration_button.visible = not inventory_manager.migration_inventory.is_empty()
		migration_button.tooltip_text = str(inventory_manager.migration_inventory)
...
func _build_ui() -> void:
	...
	migration_button.text = "Receive recovered items"
	migration_button.visible = false
	migration_button.pressed.connect(func():
		inventory_manager.claim_migration_items()
		refresh())
	vbox.add_child(migration_button)
