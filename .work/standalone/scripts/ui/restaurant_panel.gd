...
func _build_ui() -> void:
	...
	buy_table_button = Button.new()
	buy_table_button.text = "Buy next fixed table"
	buy_table_button.pressed.connect(func():
		var rest: Node = get_tree().current_scene.get_node_or_null("restaurant")
		if rest != null: rest.purchase_next_table()
		_refresh_data())
	vbox.add_child(buy_table_button)
...
func _refresh_data() -> void:
	...
	if not rest.call("is_available"):
		buy_table_button.visible = false
	...
	buy_table_button.visible = true
	buy_table_button.text = "Buy next fixed table — %s (%d/%d owned)" % [vnd_format.format_vnd(rest.get_table_purchase_cost()), rest.purchased_table_count, table_capacity]
	buy_table_button.disabled = rest.purchased_table_count >= table_capacity or not game_manager.can_afford(rest.get_table_purchase_cost())
...
func _refresh_tables(rest: Node) -> void:
	...
	for tid: String in t_ids:
		...
		if not tbl.visible: state = "not purchased"
			...
			if not jobs.has(cid):
				...
				if j_state == "cooking":
					...
					serve_btn.pressed.connect(func() -> void:
						main_world.get_node("player").begin_food_delivery(rest, cid)
						refresh()
					)
