...
func _ready() -> void:
	# A test scene must never load or autosave the normal profile, even when
	# someone launches it directly from the editor without its test runner.
	if scene_file_path.begins_with("res://tests/") and not ProjectSettings.globalize_path("user://").contains("lahoue_codex_"):
		game_manager.stop_gameplay()
		push_error("main_world: test scenes require an isolated user directory")
		get_tree().quit(1)
		return
	...
	var starting_new: bool = save_manager.is_new_game_requested()
	...
	if starting_new: save_manager.save_game()
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_CLOSE_REQUEST:
		if game_manager.gameplay_active and not save_manager.save_game():
			return
		game_manager.stop_gameplay()
		get_tree().quit()
...
func _refresh_farm_plot_visibility() -> void:
	var ground: Node2D = get_node_or_null("farm/Ground/GroundLayers/GroundVisual")
...
func _refresh_economy_world_state() -> void:
	...
	for housing_id: String in housing_levels:
		...
		if housing != null:
			...
			var visual: Node = housing.get_node_or_null("VisualRoot")
			if visual != null: visual.use_approved("buildings/%s_%d" % [housing_id,maxi(1,int(housing_levels[housing_id]))])
