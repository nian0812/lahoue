...
func _refresh_visual() -> void:
	...
	if pond_artwork != null and pond_artwork.has_method("set_artwork_enabled"):
		pond_artwork.target_rect = Rect2(-48,-48,96,84)
		pond_artwork.use_approved("buildings/pond_%d" % maxi(1,pond_level))
	...
	if species_artwork != null:
		...
		if not asset_id.is_empty() and String(species_artwork.get("semantic_id")) != asset_id:
			...
			_align_approved_water(pond_artwork,species_artwork)


func _align_approved_water(pond: Node, species: Node2D) -> void:
	if pond.artwork == null or not pond.approved_id.begins_with("buildings/pond_"): return
	var levels: Array[Vector2] = [Vector2(158,235),Vector2(555,176),Vector2(1051,162),Vector2(268,501),Vector2(994,641)]
	var manifest: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://assets/lahoue_v3/manifest.json"))
	var entry: Dictionary = manifest[pond.approved_id]
	var original: Vector2 = levels[clampi(pond_level,1,5)-1]
	var tex_point := original - Vector2(entry.crop[0]+entry.trim[0],entry.crop[1]+entry.trim[1])
	var center: Vector2 = pond.artwork.position + (tex_point-pond.artwork.texture.get_size()*0.5)*pond.artwork.scale
	species.position = center
	var observer: Node = get_node_or_null("animation_presentation")
	if observer != null: observer._base_visual_position = center
