...
func _play_clip(action: String, restart: bool = false) -> bool:
	...
	# V3 has one authored right-facing walk pair (southeast). Left-facing
	# directions (w, nw, sw) use that approved artwork horizontally mirrored.
	# Fixed-pose table clips (sit, eat, payment, cook) must remain unflipped.
	if action in ["sit", "eat", "payment", "cook"]:
		_animated.flip_h = false
	else:
		var dir_suffix: String = String(clip).get_slice("_", 1) if "_" in String(clip) else direction
		_animated.flip_h = dir_suffix in ["w", "nw", "sw"] or (dir_suffix == "default" and direction in ["w", "nw", "sw"])
