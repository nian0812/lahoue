...
func _draw() -> void:
	...
	if approved != null:
		_draw_approved_ground(approved)
		return
	match profile:
...
func _draw_approved_ground(base: Texture2D) -> void:
	var catalog = load("res://scripts/visual/lahoue_asset_catalog.gd")
	texture_filter = CanvasItem.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
	if profile == ground_profile.WORLD_GRASS:
		texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
		if material == null:
			var blend := ShaderMaterial.new()
			blend.shader = preload("res://scripts/visual/ground_blend.gdshader")
			blend.set_shader_parameter("dirt_texture",base)
			blend.set_shader_parameter("grass_texture",catalog.get_v3_texture("terrain/grass_surface"))
			material = blend
		var grass: Texture2D = catalog.get_v3_texture("terrain/grass_surface")
		draw_texture_rect(grass, area_rect, true)
	# Zone boundaries are logical only; continuous ground is supplied by the
	# world layer. Raised terrain tiles must never be scattered over this plane.


func _draw_world_grass() -> void:
	draw_rect(area_rect, Color("#66674a"))
	...
	for index: int in range(patch_count):
		...
		var patch_color: Color = (
			Color(0.27, 0.29, 0.17, rng.randf_range(0.06, 0.14))
			if index % 2 == 0
			else Color(0.62, 0.50, 0.33, rng.randf_range(0.08, 0.18))
		)
...
func _draw_farm_ground() -> void:
	...
	for row: int in range(4):
		for column: int in range(7):
			if not _is_plot_visible(row * 7 + column + 1): continue
			var center := area_rect.position + Vector2(65.0 + float(column * 60), 45.0 + float(row * 60))
			var variation: float = float((row * 7 + column + detail_seed) % 4) * 0.018
			_draw_iso_cell(
				center,
				Vector2(37.0, 19.0),
	...
	for row: int in range(3):
		...
		for column: int in range(7):
			if not _is_plot_visible((row + 1) * 7 + column + 1): continue
...
func _is_plot_visible(number: int) -> bool:
	if Engine.is_editor_hint(): return true
	var world: Node = get_tree().current_scene
	return world == null or not world.has_method("is_farm_plot_purchased") or world.is_farm_plot_purchased("farm_%02d" % number)
