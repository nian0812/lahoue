...
func _run_tests() -> void:
	...
	_expect(farm_tile_ids.size() == 28, "Farm Zone does not contain exactly 28 fixed plots")
	...
	_expect(truck_route.curve != null and truck_route.curve.point_count == 5, "Truck bay, driveway and road route was not built")
	_expect(
	...
	if truck_visuals.size() == 3:
		...
		_expect(
			(truck_manager.call("_get_truck_pos", 1.0, 0) as Vector2).is_equal_approx(
				truck_route.curve.get_point_position(4)
			),
	...
	if route_customer != null:
		...
		_expect(walk_path.size() == 5, "Customer did not receive marker route plus table destination")
		if walk_path.size() == 5:
