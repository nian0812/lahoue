...
func _validate_truck_levels_and_route() -> void:
	...
	manager.call("_process", 0.0)
	_expect(truck.visible and truck.position.is_equal_approx(manager.call("_get_truck_pos", 0.6, 0)) and truck.scale.x > 0.0, "Truck return route or non-mirrored fallback is wrong")
	deliveries[0] = {}
	manager.call("_process", 0.0)
