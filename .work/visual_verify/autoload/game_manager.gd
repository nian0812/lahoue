...
var level: int = 1
var profile_level_override: bool = false
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if OS.is_debug_build() and event.is_action_pressed("finish_day"):
...
func get_save_state() -> Dictionary:
	return {
		...
		"exp": current_exp,
		"level": level,
		"profile_level_override": profile_level_override,
...
func apply_save_state(state: Dictionary) -> void:
	...
	level = clampi(int(state.get("level", 1)), 1, data_manager.get_max_player_level())
	profile_level_override = false
	if level == data_manager.get_max_player_level(): current_exp = 0
