...
var migration_inventory: Dictionary = {}
...
func get_save_state() -> Dictionary:
	return {
		...
		"migration_inventory": migration_inventory.duplicate(true),
		"warehouse_level": warehouse_level
	}


func apply_save_state(state: Dictionary) -> void:
	migration_inventory = (state.get("migration_inventory", {}) as Dictionary).duplicate(true)
...
func claim_migration_items() -> int:
	var claimed: int = 0
	for item: String in migration_inventory.keys():
		var amount: int = mini(int(migration_inventory[item]), get_free_space())
		if amount > 0 and add_item(item, amount):
			migration_inventory[item] = int(migration_inventory[item]) - amount
			claimed += amount
		if int(migration_inventory[item]) <= 0:
			migration_inventory.erase(item)
	return claimed
