...
const max_profile_level_override: int = 100
...
var legacy_housing_allowance: Dictionary = {}
...
func _validate_save_state(state: Dictionary) -> Dictionary:
	...
	# Check migration inputs before conversions can erase invalid legacy entries.
	var layout: Dictionary = _read_integer_value(state.get("layout_version", 1), "layout_version", 1)
	if not layout.ok or int(layout.value) > 2:
		return _validation_error("unsupported layout version")
	for field: String in ["money", "restaurant_level", "kitchen_level"]:
		if not _read_integer_value(state.get(field, 0), field, 0).ok:
			return _validation_error("invalid migration field: " + field)
	for field: String in ["migration_inventory", "legacy_housing_allowance", "crops", "crop_growth"]:
		if not state.get(field, {}) is Dictionary:
			return _validation_error("invalid migration dictionary: " + field)
	if state.has("purchased_farm_plots"):
		if not state.purchased_farm_plots is Array:
			return _validation_error("purchased farm plots must be an array")
		var seen: Dictionary = {}
		for id: Variant in state.purchased_farm_plots:
			if not id is String or seen.has(id):
				return _validation_error("invalid or duplicate purchased farm plot")
			var number: int = String(id).trim_prefix("farm_").to_int()
			if id != "farm_%02d" % number or number < 1 or number > (40 if int(layout.value) == 1 else 28):
				return _validation_error("invalid purchased farm plot")
			seen[id] = true
	for id: Variant in state.get("crops", {}):
		var crop_id: Variant = state.crops[id]
		if not crop_id is String or data_manager.get_entry("crops", crop_id) == null:
			return _validation_error("invalid migration crop")
	for id: Variant in state.get("crop_growth", {}):
		if not _read_number_value(state.crop_growth[id], "crop growth", 0.0).ok:
			return _validation_error("invalid migration crop growth")
	state = preload("res://scripts/world/layout_migration.gd").migrate(state, data_manager)

	var normalized_state: Dictionary = state.duplicate(true)
	for field: String in ["migration_inventory", "legacy_housing_allowance"]:
		if not state.get(field, {}) is Dictionary:
			return _validation_error("invalid %s" % field)
		for key: Variant in state.get(field, {}):
			var amount: Dictionary = _read_integer_value(state[field][key], field, 0)
			if not bool(amount.get("ok", false)) or not key is String:
				return _validation_error("invalid %s entry" % field)
			if field == "migration_inventory" and data_manager.get_entry("items", key) == null:
				return _validation_error("unknown migration item")
	...
	var profile_level_override_value: Variant = state.get("profile_level_override", false)
	if typeof(profile_level_override_value) != TYPE_BOOL:
		return _validation_error("field 'profile_level_override' must be a boolean")
	var profile_level_override: bool = bool(profile_level_override_value)
	normalized_state["profile_level_override"] = profile_level_override
	var standard_level_is_valid: bool = (
		maximum_player_level > 0
		and level <= maximum_player_level
		and data_manager.get_level_exp(level) > 0
	)
	var profile_override_is_valid: bool = (
		profile_level_override
		and level > maximum_player_level
		and level <= max_profile_level_override
		and int(normalized_state["exp"]) == 0
	)
	if not standard_level_is_valid and not profile_override_is_valid:
		return _validation_error("field 'level' is not defined in progression data")
	# Historical opt-in test profiles load safely into normal capped progression.
	# Preserve their earned wallet, inventory and ownership; never inject a wallet.
	if profile_override_is_valid:
		normalized_state["level"] = maximum_player_level
		normalized_state["exp"] = 0
	normalized_state["profile_level_override"] = false
	if standard_level_is_valid and level < maximum_player_level and int(normalized_state["exp"]) >= data_manager.get_level_exp(level):
...
func _validate_animal_state(state: Dictionary, normalized_state: Dictionary) -> String:
	...
	for housing_id: String in housing_counts:
		...
		var allowed: int = int((state.get("legacy_housing_allowance", {}) as Dictionary).get(housing_id, 0))
		if int(housing_counts[housing_id]) > maxi(housing_capacity, allowed):
...
func _validate_restaurant_state(state: Dictionary, normalized_state: Dictionary) -> String:
	...
	var purchase_result: Dictionary = _read_integer_value(state.get("purchased_restaurant_tables", maximum_tables), "purchased_restaurant_tables", 2)
	if not bool(purchase_result.get("ok", false)) or int(purchase_result.get("value", 0)) > maximum_tables:
		return "purchased table count exceeds restaurant capacity"
	normalized_state["purchased_restaurant_tables"] = int(purchase_result.value)
	...
	for table_id_value: Variant in saved_tables:
		...
		if int(table_number_text) > int(normalized_state["purchased_restaurant_tables"]) and saved_state != restaurant_table_script.state_available:
			return "unowned table contains an active visit"
...
func _build_save_state() -> Dictionary:
	...
	state["legacy_housing_allowance"] = legacy_housing_allowance.duplicate(true)
	...
	state["purchased_restaurant_tables"] = restaurant_state.get("purchased_restaurant_tables", 2)
...
func _apply_save_state(state: Dictionary) -> void:
	legacy_housing_allowance = (state.get("legacy_housing_allowance", {}) as Dictionary).duplicate(true)
...
func create_new_game() -> void:
	legacy_housing_allowance.clear()
