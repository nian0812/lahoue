...
func _begin_leaving(requires_cleanup: bool = false) -> void:
	if restaurant != null and not table_id.is_empty():
		walk_path = restaurant.get_table_approach(table_id)
		walk_path.reverse()
		walk_path.append_array(restaurant.get_customer_exit_route())
...
func _refresh_visual() -> void:
	...
	if indicator != null:
		...
		if current_state == state_waiting_food and not order.is_empty():
			indicator_texture = asset_catalog.get_dish_texture(String(order.get("recipe_id", "")))
		if current_state == state_eating: indicator_texture = null
		...
		if indicator_texture != null:
			...
			manifest_sprite.configure_sprite(indicator, indicator_texture, Rect2(-5.0, -39.0, 10.0, 10.0))
