...
var purchased_table_count: int = 2
...
const rooftop_tables: Array[int] = [5, 6, 7, 9, 10, 11, 13, 14, 15]
const table_positions: Dictionary = {
	1: Vector2(-95, 128),
	2: Vector2(-28, 115),
	3: Vector2(-58, 142),
	4: Vector2(20, 122),
...
func _ready() -> void:
	...
	presentation.name = "ModularRestaurant"
	presentation.set_script(preload("res://scripts/visual/restaurant_presentation.gd"))
	$VisualRoot.add_child(presentation)
...
func get_save_state() -> Dictionary:
	...
	return {
		...
		"purchased_restaurant_tables": purchased_table_count,
...
func apply_save_state(saved_state: Dictionary) -> void:
	...
	purchased_table_count = int(saved_state.get("purchased_restaurant_tables", data_manager.get_restaurant_table_capacity(maxi(restaurant_level, 1))))
	...
	for customer_id_value: Variant in saved_customers_value as Dictionary:
		...
		_connect_customer_signals(customer)
		var assigned_table: Node2D = tables_by_id.get(String(customer.get("table_id"))) as Node2D
		if assigned_table != null:
			var seat_position: Vector2 = to_local(assigned_table.get_node("seat_marker").global_position)
			customer.z_index = assigned_table.z_index + 1
			if bool(customer.get("is_walking_in")):
				var approach: Array[Vector2] = get_table_approach(String(customer.get("table_id")))
				approach.append(seat_position)
				customer.set("walk_path", approach)
			elif not bool(customer.get("is_walking_out")):
	...
	for table_id_value: Variant in tables_by_id.keys():
		...
		if number_text.is_valid_int() and int(number_text) > target_capacity:
			...
			if removed_table != null and is_instance_valid(removed_table):
				removed_table.free()
	for id: String in tables_by_id:
		var table: Node2D = tables_by_id[id]
		var number: int = id.trim_prefix("table_").to_int()
		table.visible = number <= purchased_table_count
		table.position = _get_upgrade_table_position(number)
		table.z_index = 2 if rooftop_tables.has(number) else 0
		table.set_meta("floor", "rooftop" if rooftop_tables.has(number) else "ground")
	return tables_by_id.size() == target_capacity


func get_table_purchase_cost() -> int:
	return int((data_manager.get_dataset("progression").get("restaurant", {}) as Dictionary).get("table_purchase_cost", 100000))


func purchase_next_table() -> bool:
	if not is_available() or purchased_table_count >= data_manager.get_restaurant_table_capacity(restaurant_level): return false
	if not game_manager.spend_money(get_table_purchase_cost()): return false
...
func _get_upgrade_table_position(table_number: int) -> Vector2:
	var result: Vector2
	if table_positions.has(table_number):
		result = table_positions[table_number]
	else:
		var roof_index: int = rooftop_tables.find(table_number)
		if roof_index >= 0:
			# These coordinates are the V3 rooftop's authored empty table markers.
			result = Vector2(-104, -85) + Vector2(48, -18) * (roof_index % 3) + Vector2(43, 21) * (roof_index / 3)
		else:
			var ground_index: int = [1, 2, 3, 4, 8, 12].find(table_number)
			if ground_index < 0: ground_index = table_number - 1
			# Keep the stair landing to the west clear; the right-hand open floor is
			# the final ground-table slot.
			result = Vector2(-100 + (ground_index % 3) * 78, 115 + (ground_index / 3) * 60)
	var slot: Marker2D = get_node_or_null("ExpansionAnchors/TableSlots/TableSlot%02d" % table_number) as Marker2D
	if slot != null: slot.position = result
	return result


func get_table_approach(table_id: String) -> Array[Vector2]:
	if rooftop_tables.has(table_id.trim_prefix("table_").to_int()):
		# The approved V3 building's staircase is on the west façade. Keep the
		# landing and its foot clear while routing every rooftop visit through it.
		return [Vector2(-205, 80), Vector2(-212, 34), Vector2(-166, -126), Vector2(-132, -143)]
	return [Vector2(-140, 120)]
...
func _begin_customer_visit(customer_id: String) -> void:
	...
	arr.append_array(get_table_approach(table_id))
	arr.append(seat_pos)
...
func _on_customer_arrived_at_table(customer_id: String) -> void:
	...
	if table == null:
		return
	customer.z_index = (table as Node2D).z_index + 1
...
func _is_staff_job_valid(job_type: String, target_id: String, ignored_claim_key: String = "") -> bool:
	...
	match job_type:
		...
		staff_script.job_serve:
			...
			if player != null and player.get("delivery_customer_id") == target_id: return false
...
func _find_available_table() -> Node:
	...
	for table_id_value: Variant in table_ids:
		...
		if table.visible and String(table.get("current_state")) == restaurant_table_script.state_available:
			return table
	return null
