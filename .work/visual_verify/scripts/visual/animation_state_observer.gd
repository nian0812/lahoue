...
func _present_carried_meal() -> void:
	...
	var animated: AnimatedSprite2D = _visual_root.get_node_or_null("animation_artwork") as AnimatedSprite2D
	if animated != null and animated.visible:
		is_flipped = animated.flip_h
	var meal_rect: Rect2 = Rect2(-14, -15, 14, 12) if is_flipped else Rect2(0, -15, 14, 12)
	preload("res://scripts/visual/manifest_sprite.gd").configure_sprite(_carried_meal, texture, meal_rect)
...
func refresh_snapshot() -> void:
	...
	if not motion.is_zero_approx():
		_direction = character_direction_from_vector(motion) if actor_kind in ["player", "customer"] else direction_from_vector(motion)
...
static func character_direction_from_vector(value: Vector2) -> String:
	# For strict diagonals with equal x and y, preserve exact isometric quadrant contract.
	if is_equal_approx(absf(value.x), absf(value.y)):
		if value.y < 0.0:
			return "ne" if value.x >= 0.0 else "nw"
		return "se" if value.x >= 0.0 else "sw"
	return direction_from_vector(value)
...
func _present_staff_route() -> void:
	...
	_visual_root.z_index = 3 if point.y < -80 or String((_actor.get("active_job") as Dictionary).get("job_type", "")) == "serve" else 0
