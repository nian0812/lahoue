...
static var _v3_frames: Dictionary = {}


static func get_v3_texture(id: String) -> Texture2D:
	var path := "res://assets/lahoue_v3/%s.png" % id
	if not ResourceLoader.exists(path): return null
	if not _texture_cache.has(path): _texture_cache[path] = load(path)
	return _texture_cache[path] as Texture2D


static func get_v3_frames(id: String) -> SpriteFrames:
	if _v3_frames.has(id): return _v3_frames[id]
	var folder: String = "characters" if id in ["player","waiter","chef","customer","customer_woman","vip_customer"] else "animals" if id in ["layer_chicken","meat_chicken","pig","dairy_cow","beef_cow"] else "aquaculture"
	if get_v3_texture("%s/%s/idle" % [folder,id]) == null: return null
	var frames := SpriteFrames.new()
	frames.remove_animation("default")
	for clip: String in ["idle", "walk", "carry", "sit", "seated_idle", "eat", "serve", "payment", "cook"]:
		var source_id: String = {"seated_idle":"sit", "serve":"carry", "cook":"carry"}.get(clip,clip)
		var sources: Array = ["walk_0","walk_1"] if clip == "walk" else [source_id]
		if get_v3_texture("%s/%s/%s" % [folder,id,sources[0]]) == null: continue
		var animation_names: Array[StringName] = [StringName(clip + "_default")]
		# The approved restaurant sheet supplies one authored three-quarter walk
		# pair (facing southeast). Expose all 8 movement directions explicitly;
		# the presenter mirrors leftward movements (w, nw, sw) with flip_h.
		if folder == "characters":
			...
			elif clip in ["idle", "carry"]:
				animation_names = [
					StringName(clip + "_default"),
					StringName(clip + "_e"), StringName(clip + "_se"), StringName(clip + "_s"), StringName(clip + "_sw"),
					StringName(clip + "_w"), StringName(clip + "_nw"), StringName(clip + "_n"), StringName(clip + "_ne")
				]
		for animation: StringName in animation_names:
			frames.add_animation(animation)
			frames.set_animation_speed(animation, 5.0 if folder == "characters" else 2.0)
			if clip in ["sit", "serve", "payment", "cook"]:
				frames.set_animation_loop(animation, false)
				frames.set_animation_speed(animation, 1.25)
			for source: String in sources: frames.add_frame(animation, get_v3_texture("%s/%s/%s" % [folder,id,source]))
	_v3_frames[id] = frames
	return frames


static func get_manifest_texture(manifest_id: String, section_id: String, semantic_id: String) -> Texture2D:
	var v3_id: String = ""
	if manifest_id in ["characters", "animals", "aquaculture"]: v3_id = "%s/%s/idle" % [manifest_id,semantic_id]
	if manifest_id == "characters": v3_id += "_static"
	if semantic_id == "empty_farm_plot": v3_id = "terrain/plot"
	if not v3_id.is_empty():
		var approved: Texture2D = get_v3_texture(v3_id)
