...
func use_approved(id: String) -> bool:
	var texture: Texture2D = asset_catalog.get_v3_texture(id)
	if texture == null: return false
	if artwork == null: refresh_artwork()
	if artwork == null: return false
	approved_id = id
	artwork.texture = texture
	var rect: Rect2 = target_rect
	if rect.size.x <= 0.0 or rect.size.y <= 0.0:
		var size: Vector2 = get_parent().get("building_size")
		rect = Rect2(-size.x*0.5,-size.y,size.x,size.y)
	_fit_bottom_centered(artwork, rect)
	return true
