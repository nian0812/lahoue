...
var bar_prop: Sprite2D
var pass_prop: Sprite2D
var food_pass_position := Vector2(-22, -55)
...
func _ready() -> void:
	...
	pass_prop = _approved_prop("food_pass",Rect2(-35,-86,72,60))
	bar_prop = _approved_prop("bar",Rect2(-145,-110,80,65))
	kitchen_label = _label("Kitchen",Vector2(88,-82))
...
func refresh() -> void:
	...
	if kitchen != null:
		...
		kitchen.visible = not shell.visible
		pass_prop.visible = not shell.visible
		bar_prop.visible = not shell.visible
	...
	for dish: Node2D in ready_dishes.values():
		...
		dish.position = food_pass_position + Vector2((index % 3) * 16, (index / 3) * 10)
		index += 1
...
func _draw() -> void:
	...
	_material_face([Vector2(-240,25),Vector2(0,-120),Vector2(240,25),Vector2(0,220)], "floor_material")
	_material_face([Vector2(-240,25),Vector2(0,220),Vector2(0,229),Vector2(-240,34)], "wall_material")
	_material_face([Vector2(0,220),Vector2(240,25),Vector2(240,34),Vector2(0,229)], "wall_material")
	if floor_level >= 3:
...
func _set_approved_shell() -> void:
	var level: int = clampi(maxi(floor_level, 1), 1, 5)
	var texture: Texture2D = catalog.get_v3_texture("restaurant/level_%d" % level)
	if texture == null:
		shell.visible = false
		return
	# This is the unmodified, approved V3 progression artwork. It supplies the
	# roof, stairs, rails, lights and empty fixed table slots; runtime tables,
	# customers and dishes remain separate gameplay actors above it.
	sprite_tools.configure_sprite(shell, texture, Rect2(-255, -270, 510, 480))
	shell.visible = true
	if level < 3:
		bar.position = Vector2(-85, -5)
		pass_label.position = Vector2(10, -5)
		kitchen_label.position = Vector2(112, -16)
		food_pass_position = Vector2(18, 10)
	else:
		bar.position = Vector2(-94, -140)
		pass_label.position = Vector2(-10, -140)
		kitchen_label.position = Vector2(92, -140)
		food_pass_position = Vector2(12, -122)
