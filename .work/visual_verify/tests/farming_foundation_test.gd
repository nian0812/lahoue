...
func _run_tests() -> void:
	...
	_expect(world.farm_tiles_by_id.size() == 28, "main_world did not register all 28 farm plots")
	_expect(_farm_tile_child_count() == 28, "Farm grid contains duplicate or missing plot nodes")
	...
	_expect(_grid_layout_is_valid(), "28 plots are not arranged as the fixed 7x4 Farm grid")
	...
	for _purchase_index: int in range(remaining_purchases):
		_expect(bool(expansion_point.call("interact", player)), "Farm Expansion failed before plot #28")
	_expect((world.get("purchased_farm_plots") as Array).size() == 28, "Farm Expansion did not stop at 28 plots")
	_expect(_visible_plot_count() == 28, "not all 28 purchased plots are visible and usable")
	_expect(game_manager.money == 0, "Farm Expansion maximum charged the wrong total")
	prompt.call("_update_text", expansion_point)
	_expect(String((prompt.get("prompt_label") as Label).text) == "Farm Plots: 28/28\nFarm fully expanded", "maximum Farm Expansion feedback is wrong")
	game_manager.money = data_manager.get_farm_plot_purchase_cost()
	_expect(not bool(expansion_point.call("interact", player)), "Farm Expansion purchased plot #29")
	_expect(game_manager.money == data_manager.get_farm_plot_purchase_cost(), "fully expanded Farm charged money again")
	_expect(_farm_tile_child_count() == 28, "maximum Farm Expansion duplicated plot nodes")
