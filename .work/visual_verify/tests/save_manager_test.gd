...
func _run_tests() -> void:
	...
	var profile_override_state: Dictionary = game_manager.get_save_state()
	profile_override_state["day"] = 7
	profile_override_state["day_timer"] = 0.0
	profile_override_state["level"] = 100
	profile_override_state["exp"] = 0
	profile_override_state["money"] = 1000000000
	profile_override_state["profile_level_override"] = true
	game_manager.apply_save_state(profile_override_state)
	_expect(save_manager.save_game(), "explicit Lv100 profile override could not be saved")
	game_manager.apply_save_state({})
	_expect(save_manager.load_game(), "explicit Lv100 profile override could not be loaded")
	_expect(game_manager.level == 55 and not game_manager.profile_level_override, "legacy test level did not normalize to real cap")
	_expect(game_manager.money == 1000000000, "one-billion wallet was not restored")
