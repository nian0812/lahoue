...
func _build_bottom_bar() -> void:
	...
	exp_bar.custom_minimum_size = Vector2(72, 10)
	...
	inv_label = _make_label("0/100", 13)
...
func _on_money_changed(p_money: Variant) -> void:
	money_label.text = vnd_format.format(int(p_money))
	_animate_label(money_label)


func _on_level_changed(p_level: Variant) -> void:
	level_label.text = "Lv %d" % int(p_level)
...
func _on_reputation_changed(p_reputation: Variant) -> void:
	rep_label.text = "%.1f" % float(p_reputation)


func _on_capacity_changed(p_current: Variant, p_maximum: Variant) -> void:
	inv_label.text = "%d/%d" % [int(p_current), int(p_maximum)]
...
func _on_seed_changed(p_seed_item_id: Variant) -> void:
	...
	if seed_id.is_empty():
		seed_label.text = "—"
		return
	seed_label.text = vnd_format.format_item_name(seed_id)
