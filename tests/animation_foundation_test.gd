...
func _run() -> void:
	...
	var all_8_directions: Dictionary = {
		Vector2(1, 0): "e",
		Vector2(1, 1): "se",
		Vector2(0, 1): "s",
		Vector2(-1, 1): "sw",
	...
	for motion: Vector2 in all_8_directions:
		var direction: String = all_8_directions[motion]
		_expect(observer_script.character_direction_from_vector(motion) == direction, "8-direction character motion: %s" % direction)
		var expect_flip: bool = direction in ["w", "nw", "sw"]
		...
		_expect(animated != null and animated.flip_h == expect_flip, "customer facing: %s" % direction)
		customer_observer.present({"identity":"customer", "state":"idle", "direction":direction})
		_expect(animated != null and animated.flip_h == expect_flip, "customer idle facing: %s" % direction)
		...
		_expect(player_animated != null and player_animated.flip_h == expect_flip, "player facing: %s" % direction)
		player_observer.present({"identity":"player", "state":"idle", "direction":direction})
		_expect(player_animated != null and player_animated.flip_h == expect_flip, "player idle facing: %s" % direction)
